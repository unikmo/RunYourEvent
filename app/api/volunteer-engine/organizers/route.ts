...
export async function POST(req:NextRequest){
  try{
    ...
    if(pipeline==='one_off_event'){
      if(!eventDate||!shiftStart||!shiftEnd||text(body.venue,240).length<2||!transportModes.includes(transportMode)||body.transportAck!==true){
    ...
    const {data,error}=await db.rpc('rye_submit_volunteer_organizer_request_v3',{
      p_organization_name:organizationName,p_contact_name:contactName,p_contact_email:contactEmail,p_phone:text(body.phone,60)||null,p_city:city,p_postal_code:text(body.postalCode,30)||null,p_pipeline:pipeline,p_event_type:text(body.eventType,120)||null,p_event_name:text(body.eventName,160)||null,p_event_date:eventDate,p_season_label:text(body.seasonLabel,100)||null,p_volunteers_needed:volunteersNeeded,p_role_examples:text(body.roleExamples,1500)||null,p_notes:text(body.notes,2000)||null,p_responsibility_ack:true,p_coverage_status:coverageStatus,p_protection_selected:protectionSelected,p_protection_ack:protectionAck,
      p_venue:text(body.venue,240)||null,p_shift_start_local:shiftStart,p_shift_end_local:shiftEnd,p_transport_mode:transportMode,p_transport_details:text(body.transportDetails,1000)||null,p_transport_ack:body.transportAck===true,p_commercial_ack:true,
    })
